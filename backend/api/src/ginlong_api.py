...
CHECK = set(
    (INVERTER_STATE, INVERTER_TIMESTAMP_UPDATE, INVERTER_SERIAL, INVERTER_ENERGY_TODAY)
)
        ...
        if result[SUCCESS] is True:
            ...
            try:
                if result_json["result"].get("isAccept") == 1:
                    ...
                    # Fetch plant name
                    data = await self.fetch_inverter_data(
                        next(iter(self._inverter_list))
                    )
            ...
            try:
                for record in reversed(result_json["result"]["paginationAjax"]["data"]):
                    ...
                    if active or update_date > two_days_ago:
                        ...
                        _LOGGER.warning(
                            "Inverter %s inactive more than 48hrs, ignoring", serial
                        )
            except TypeError:
    ...
    def _collect_inverter_data(self, payload: dict[str, Any]) -> bool:
        ...
        for subkey in INVERTER_DATA:
            ...
            for dictkey in attributes:
                ...
                if key is not None:
                    value, unit = getattr(self, methodname)(
                        jsondata, key, type_, precision
                    )
