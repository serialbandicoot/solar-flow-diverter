...
from backend.api.src.ginlong_api import BaseAPI, GinlongData, PortalConfig
from backend.api.src.ginlong_const import *
...
class SoliscloudAPI(BaseAPI):
    ...
    async def fetch_inverter_data(self, inverter_serial: str) -> GinlongData | None:
        ...
        if self.is_online:
            if (
                self._inverter_list is not None
                and inverter_serial in self._inverter_list
            ):
    ...
    async def _get_inverter_details(
    ...
    def _collect_inverter_data(self, payload: dict[str, Any]) -> None:
        ...
        try:
            collect_energy_today = not self.config.workarounds[
                "use_energy_today_from_plant"
            ]
        except KeyError:
            pass
    ...
    def _collect_station_list_data(self, payload: dict[str, Any]) -> None:
        ...
        try:
            collect_energy_today = self.config.workarounds[
                "use_energy_today_from_plant"
            ]
        except KeyError:
            pass
    ...
    def _post_process(self) -> None:
        ...
        if self._data:
            ...
            self._fix_units(
                BAT_TOTAL_ENERGY_DISCHARGED, BAT_TOTAL_ENERGY_DISCHARGED_STR
            )
            self._fix_units(
                GRID_TOTAL_CONSUMPTION_POWER, GRID_TOTAL_CONSUMPTION_POWER_STR
            )
            ...
            self._fix_units(
                GRID_TOTAL_ENERGY_PURCHASED, GRID_TOTAL_ENERGY_PURCHASED_STR
            )
            ...
            self._fix_units(
                GRID_DAILY_ENERGY_PURCHASED, GRID_DAILY_ENERGY_PURCHASED_STR
            )
            self._fix_units(
                GRID_MONTHLY_ENERGY_PURCHASED, GRID_MONTHLY_ENERGY_PURCHASED_STR
            )
            self._fix_units(
                GRID_YEARLY_ENERGY_PURCHASED, GRID_YEARLY_ENERGY_PURCHASED_STR
            )
    ...
    def _prepare_header(
        self, body: dict[str, str], canonicalized_resource: str
    ) -> dict[str, str]:
        ...
        hmac_obj = hmac.new(
            self.config.secret, msg=encrypt_str.encode("utf-8"), digestmod=hashlib.sha1
        )
    ...
    async def _post_data_json(
        self, canonicalized_resource: str, params: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error from URI (%s) : %s", canonicalized_resource, result[MESSAGE]
            )
        finally:
            if resp is not None:
                await resp.release()
